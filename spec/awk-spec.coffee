...
describe "LanguageAwk", ->
  ...
  it "tokensizes regexp", ->
    ...
    expect(tokens[3]).toEqual value: '^', scopes: ['source.awk', 'string.regexp.awk', 'keyword.other.awk']
    expect(tokens[4]).toEqual value: '[', scopes: ['source.awk', 'string.regexp.awk', 'keyword.other.awk', 'punctuation.keyword.other.begin.awk']
    expect(tokens[5]).toEqual value: '[:alpha:]', scopes: ['source.awk', 'string.regexp.awk', 'keyword.other.awk', 'support.class.awk']
    expect(tokens[6]).toEqual value: ']', scopes: ['source.awk', 'string.regexp.awk', 'keyword.other.awk', 'punctuation.keyword.other.end.awk']
    expect(tokens[7]).toEqual value: '+', scopes: ['source.awk', 'string.regexp.awk', 'keyword.other.awk']
